    ...
    public function exportAll(){
        ...
        foreach(\MkmScraper\Card::where("rarity","Mythic")->get() as $card) {
            foreach ($card->graphPrices as $key => $price) {
                $row = array();
                $row[] = $price->priceClass();
                $row[] = $price->card->set->daysFromRelease();
                $row[] = $price->card->set->daysFromRotation();
                $row[] = $price->tournamentLastWeek();
                $row[] = $price->tournamentLastTwoWeek();
                $row[] = $price->tournamentLastThreeWeek();
                $row[] = $price->articlesLastWeek();
                $row[] = $price->articlesLastTwoWeek();
                $row[] = $price->articlesLastThreeWeek();
                $row[] = $price->otherCardMovementDay();
                $row[] = $price->otherCardMovementWeek();
                $row[] = $price->boostersOpen();
                $array[] = $row;
